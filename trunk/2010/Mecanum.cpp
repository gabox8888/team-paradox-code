...
CRITICAL_REGION(m_mutexSemaphore)
{
	if ( m_isEnabled && DriverStation::GetInstance()->IsEnabled() )
	{
		...
		// PGV: Hey Chris, I moved the deadzone test here.  "m_setpoint" should be near zero when the joystick is centered on all axes...
		if (fabs(m_setpoint) < 0.08)
		{
			m_pOutputSpeedController->Set(0.0f);
			m_errorIntegral *= 0.95f; // decay the error intergral while the joystick is centered so we don't unexpectedly get a "kick" when we move off-center.
		}
		else
		{
...
protected:
	...
	// true when we're preocessing in autonomous mode.  We keep this variable to detect the transition from autonomous to teleoperated mode.
	// this lets us reset variables that were altered in autonomous mode...
	bool                  m_bProcessingAutonomous;
	
	// Various and sundry autonomous mode state variables...
	float                 m_autoDrive_X;
	float                 m_autoDrive_Y;
	float                 m_autoDrive_Z;
...
//	printf("robot thread prio=%d\n", prio);
	...
	*/
	...
	const bool s_flipWheel_And_Tower = false;
...
*/
	...
	// Hey Chris, I moved your deadzone test into the ParadoxSpeedController::Calculate function.  The problem with doing the test here is that
	// the speed controller is running concurrently in another task and will continue to set non-zero PWM values even when you're setting them
	// to zero here...
...
*/
	...
	// So many states use the drive timer, we'll just always have it running...
	m_driveTimer -= m_dT;
	if (m_driveTimer < 0.0f)
	{
		m_driveTimer = 0.0f;
	}
