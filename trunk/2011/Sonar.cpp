...
const double Sonar::kPingTime;	///< Time (sec) for the ping trigger pulse.
const UINT32 Sonar::kPriority;	///< Priority that the sonar round robin task runs.
const double Sonar::kMaxSonarTime;	///< Max time (ms) between readings.
const double Sonar::kSpeedOfSoundInchesPerSec;
Task Sonar::m_task("SonarChecker", (FUNCPTR)SonarChecker); // task doing the round-robin automatic sensing
Sonar *Sonar::m_firstSensor = NULL; // head of the sonar sensor list
bool Sonar::m_automaticEnabled = false; // automatic round robin mode
SEM_ID Sonar::m_semaphore = 0;

/**
 * Background task that goes through the list of sonar sensors and pings each one in turn. The counter
 * is configured to read the timing of the returned echo pulse.
 * 
 * DANGER WILL ROBINSON, DANGER WILL ROBINSON:
 * This code runs as a task and assumes that none of the sonar sensors will change while it's
 ...
 * This is the common code that initializes the sonar sensor given that there
...
/**
 * Create an instance of the Sonar Sensor using the default module.
 * This is designed to supchannel the Daventech SRF04 and Vex sonar sensors. This
 ...
 */
Sonar::Sonar(UINT32 pingChannel, UINT32 echoChannel, DistanceUnit units)
{
...
/**
 * Create an instance of an Sonar Sensor from a DigitalInput for the echo channel and a DigitalOutput
 ...
 * @param units The units returned in either kInches or kMilliMeters
 */
Sonar::Sonar(DigitalOutput *pingChannel, DigitalInput *echoChannel, DistanceUnit units)
{
...
/**
 * Create an instance of the Sonar sensor using specified modules.
 * This is designed to supchannel the Daventech SRF04 and Vex sonar sensors. This
 ...
 * @param units The units returned in either kInches or kMilliMeters
 */
Sonar::Sonar(UINT32 pingSlot, UINT32 pingChannel,
...
/**
 * Destructor for the sonar sensor.
 * Delete the instance of the sonar sensor by freeing the allocated digital channels.
...
Sonar::~Sonar()
{
	...
	semTake(m_semaphore, WAIT_FOREVER);
	{
		...
		else
		{
			...
			for (Sonar *s = m_firstSensor; s != NULL; s = s->m_nextSensor)
			{
 ...
 * @param enabling Set to true if round robin scheduling should start for all the sonar sensors. This
 ...
 */
void Sonar::SetAutomaticMode(bool enabling)
{
	...
	if (enabling)
	{
		...
		for (Sonar *u = m_firstSensor; u != NULL; u = u->m_nextSensor)
		{
			u->m_counter->Reset();
		}
...
/**
 * Single ping to sonar sensor.
 * Send out a single ping to the sonar sensor. This only works if automatic (round robin)
 ...
 */
bool Sonar::IsRangeValid()
{
	return m_counter->Get() > 1;
}

/**
 * Get the range in inches from the sonar sensor.
 * @return double Range in inches of the target returned from the sonar sensor. If there is
 * no valid value yet, i.e. at least one measurement hasn't completed, then return 0.
 */
double Sonar::GetRangeInches()
{
	if (IsRangeValid())
...
/**
 * Get the range in millimeters from the sonar sensor.
 * @return double Range in millimeters of the target returned by the sonar sensor.
 * If there is no valid value yet, i.e. at least one measurement hasn't complted, then return 0.
 */
double Sonar::GetRangeMM()
{
 ...
 */
void Sonar::SetDistanceUnits(DistanceUnit units)
{
	m_units = units;
}
 ...
 */
Sonar::DistanceUnit Sonar::GetDistanceUnits()
{
	return m_units;
}
